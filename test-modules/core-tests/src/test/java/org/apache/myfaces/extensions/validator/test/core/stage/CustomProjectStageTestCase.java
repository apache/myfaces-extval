...
public class CustomProjectStageTestCase extends AbstractExValCoreTestCase
{
    ...
    public static class CustomProjectStageResolver extends DefaultProjectStageResolver
    {

        public ProjectStage getCurrentProjectStage()
        {
            // don't use the default extval mechanism to avoid too early
            // evaluation with mojarra
            String result = null;
            try
            {
                result = FacesContext.getCurrentInstance().getExternalContext().getInitParameter(PROJECT_STAGE);
                if(result != null)
                {
                    result = result.trim();
                    if(!"".equals(result))
                    {
                        return createProjectStage(ProjectStage.createStageName(result));
                    }
                }
            } catch (Exception e)
            {
            }
            return super.getCurrentProjectStage();
        }
